...
class OpenAiWrapper {
    ...
    async postMessage(threadId, message) {
        logging_1.logger.d("Posting message...");
            ...
            while (false === complete) {
                ...
                switch (status) {
                    case "completed":
                        ...
                        continue;
                    case "cancelled":
                        throw new ChatError_1.ChatError("cancelled", true, `Thread run error. Status: cancelled, Error: ${run.last_error}`);
                    case "failed":
                        throw new ChatError_1.ChatError("internal", true, `Thread run error. Status: failed, Error: ${run.last_error}`);
                    case "expired":
                        throw new ChatError_1.ChatError("deadline-exceeded", true, `Thread run error. Status: expired, Error: ${run.last_error}`);
                    case "requires_action":
                        ...
                        switch (requiredActionType) {
                            case "submit_tool_outputs":
                                ...
                                break;
                            default:
                                throw new ChatError_1.ChatError("internal", true, `Unknown action: ${requiredActionType}`);
                        }
                        break;
                    default:
                        throw new ChatError_1.ChatError("unimplemented", true, `Thread run error - unknown status. Status: ${status}`);
                }
            }
            return data;
            /**
    ...
    async runAi(block) {
        try {
            ...
            return Promise.reject(new ChatError_1.ChatError("unavailable", false, "Error running AI", e));
        }
    }
}
