...
class ChatWorker {
    ...
    async runPostChat(state, command) {
        logging_1.logger.d(`Inserting messages. runId ${command.dispatchId}, doc: ${command.doc}`);
        const messageCollectionRef = command.doc.collection(Collections_1.Collections.messages);
        ...
        if (0 === toPost.length) {
            await this.updateIfChecked("processing", command, (tx, state) => {
                ...
                tx.set(command.doc, Object.assign(Object.assign({}, newState), { updatedAt: FieldValue.serverTimestamp() }), { merge: true });
                return newState;
            });
            return;
        }
        ...
        if (undefined === threadId) {
            logging_1.logger.d(`Creating thread. runId ${command.dispatchId}, doc: ${command.doc}`);
        ...
        // 3. Get new messages
        logging_1.logger.d(`Getting messages, runId ${command.dispatchId}, doc: ${command.doc}`);
    ...
    async updateWithCheck(status, command, block) {
        return await this.db.runTransaction(async (tx) => {
            ...
            if (status !== state.status || command.dispatchId !== state.dispatchId) {
                return Promise.reject(new https_1.HttpsError("failed-precondition", "Chat status conflict"));
            }
    ...
    async processWithCheck(status, command, block) {
        logging_1.logger.d(`Processing command: ${command.type}, runId ${command.dispatchId}, doc: ${command.doc}`);
        const run = this.updateWithCheck(status, command, (tx, state) => {
            const newState = Object.assign(Object.assign({}, state), { status: "processing" });
            tx.set(command.doc, Object.assign(Object.assign({}, newState), { updatedAt: FieldValue.serverTimestamp() }));
            return newState;
        });
        let state;
        try {
        ...
        try {
            ...
            await command.doc.set({ status: "failed", updatedAt: FieldValue.serverTimestamp() }, { merge: true });
        }
    }
}
