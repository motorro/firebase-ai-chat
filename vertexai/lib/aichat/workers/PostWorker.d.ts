import { ChatCleaner, ChatCommand, ChatData, ChatState, Continuation, DispatchControl, TaskScheduler, ToolCallRequest, ToolCallsResult, ToolContinuationDispatcherFactory } from "@motorro/firebase-ai-chat-core";
...
declare abstract class BasePostWorker extends VertexAiQueueWorker {
     ...
     */
    constructor(firestore: FirebaseFirestore.Firestore, scheduler: TaskScheduler, wrapper: AiWrapper, instructions: Readonly<Record<string, VertexAiSystemInstructions<any>>>, getDispatcherFactory: () => ToolContinuationDispatcherFactory, cleaner: ChatCleaner, logData: boolean);
    /**
     * Runs post action
     ...
     * @protected
     */
    protected abstract doPost(command: VertexAiChatCommand, threadId: string, dispatcherId: string, instructions: VertexAiSystemInstructions<ChatData>, soFar: ChatData, updateStateData: (data: ChatData) => Promise<boolean>): Promise<Continuation<PostMessageResult<ChatData>>>;
    /**
     ...
     * @protected
     */
    protected getPostDispatch(command: VertexAiChatCommand, dispatcherId: string, updateData: (data: ChatData) => Promise<boolean>): (data: ChatData, toolCalls: ReadonlyArray<ToolCallRequest>) => Promise<Continuation<ToolCallsResult<ChatData>>>;
...
export declare class PostWorker extends BasePostWorker {
    ...
    protected doPost(command: VertexAiChatCommand, threadId: string, dispatcherId: string, instructions: VertexAiSystemInstructions<ChatData>, soFar: ChatData, updateStateData: (data: ChatData) => Promise<boolean>): Promise<Continuation<PostMessageResult<ChatData>>>;
}
...
export declare class ContinuePostWorker extends BasePostWorker {
    ...
    protected doPost(command: VertexAiContinuationCommand, threadId: string, dispatcherId: string, instructions: VertexAiSystemInstructions<ChatData>, soFar: ChatData, updateStateData: (data: ChatData) => Promise<boolean>): Promise<Continuation<PostMessageResult<ChatData>>>;
}
export {};
